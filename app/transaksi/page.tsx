...
import { IconHistory, IconCirclePlus, IconCalendar, IconTrendingUp, IconUsers, IconCreditCard, IconPackage } from "@tabler/icons-react";
...
async function getTransactionStats() {
  ...
  const [todayCount, monthCount, totalMembers, pendingTransactions] = await Promise.all([
...
async function getChartData() {
  ...
  const today = new Date();
  const daysInPeriod = Math.ceil((today.getTime() - oneMonthAgo.getTime()) / (1000 * 60 * 60 * 24));
    ...
    >
      ...
      <SidebarInset>
              ...
              <div className="px-4 lg:px-6">
                <div className="flex items-end justify-between mb-6">
                  <div>
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mt-1">Kelola transaksi dan pantau performa bisnis Anda</p>
                  </div>
                </div>
          ...
          <Card className="bg-gradient-to-br from-emerald-500 to-emerald-600 text-white border-0">
            <CardContent className="p-6">
              <div className="flex flex-col space-y-2">
                ...
                <div>
                  <p className="text-emerald-100 text-sm font-medium">Bulan Ini</p>
                  <p className="text-emerald-100 text-xs">Transaksi</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-purple-500 to-purple-600 text-white border-0">
            <CardContent className="p-6">
              <div className="flex flex-col space-y-2">
                ...
                <div>
                  <p className="text-purple-100 text-sm font-medium">Total Member</p>
        ...
        <Card className="lg:col-span-4">
          ...
          <CardContent>
            <MyForm outlets={formData.outlets} members={formData.members} packages={formData.packages}/>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-3 max-h-[800px] overflow-auto">
              {recentTransactions.map((t) => (
                <div key={t.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50 hover:bg-muted transition">
                  <div className="flex-1">
                    ...
                    <p className="text-sm font-medium">{t.tb_member?.nama || 'Guest'}</p>
                    <p className="text-xs text-muted-foreground">
                      {format(new Date(t.tgl), 'dd/MM/yyyy HH:mm')}
                    </p>
                  </div>
                  <Badge variant={t.status === 'selesai' ? 'default' : t.status === 'proses' ? 'secondary' : t.status === 'diambil' ? 'outline' : 'destructive'}>
                    {t.status}
                  </Badge>
                </div>
              ))}
