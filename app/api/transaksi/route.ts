  ...
  try {
    ...
    if (!id_outlet) {
      return NextResponse.json(
        { success: false, error: "Outlet wajib dipilih." },
        { status: 400 }
      );
    }
    if (!items || items.length === 0) {
      return NextResponse.json(
        { success: false, error: "Tambahkan minimal 1 paket." },
        { status: 400 }
      );
    }
    ...
    if (diskon_type === "fixed") {
      ...
      parsedDiskon =
        base > 0 ? Math.min(100.0, (parsedDiskon / base) * 100.0) : 0.0;
    }
    ...
    // Revalidate halaman transaksi dan home agar data terupdate
